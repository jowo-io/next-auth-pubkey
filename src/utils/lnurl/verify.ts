...
function isHex(value) {
  assert.strictEqual(
    typeof value,
    "string",
    'Invalid argument ("value"): String expected.'
  );
  return Buffer.from(value, "hex").toString("hex") === value.toLowerCase();
}

export default function verify(sig, k1, key) {
  ...
  assert.ok(
    Buffer.isBuffer(sig),
    'Invalid argument ("sig"): Hex-encoded string or buffer expected.'
  );
  ...
  assert.ok(
    Buffer.isBuffer(k1),
    'Invalid argument ("k1"): Hex-encoded string or buffer expected.'
  );
  ...
  assert.ok(
    Buffer.isBuffer(key),
    'Invalid argument ("key"): Hex-encoded string or buffer expected.'
  );
